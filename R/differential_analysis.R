...
#' @examples
#' \dontrun{
#' # Simple cluster markers
#' markers <- run_DEG(seurat_obj, groupBy = "seurat_clusters")
#' 
#' # Complex comparison: Treatment vs Control within each cell type
#' compare_df <- data.frame(
#'   groupA = c("Treatment", "Treatment"),
#'   groupB = c("Control", "Vehicle")
#' )
#' markers <- run_DEG(seurat_obj,
#'                    groupBy = "cell_type",
#'                    compareBy = "condition",
#'                    compareDF = compare_df)
#' }
run_DEG <- function(seuObj, groupBy, groupIDs = NULL, compareBy = NULL, 
                    compareDF = NULL, testUse = "wilcox", recorrectUMI = FALSE,
                    onlyPos = TRUE, assayName = "SCT") {
  ...
  if (!is.null(compareBy) & !is.null(compareDF)) {
    ...
    for (i in seq(1, length(groupIDs))) {
      ...
      for (j in seq(1, nrow(compareDF))) {
        ...
        cat("[", formatted_time, "] Starting FindMarkers: ", groupIDs[i], " - ", 
            compareA, " vs ", compareB, "\n", sep = "")
        ...
        if (cellNumA >= 3 & cellNumB >= 3) {
          df <- FindMarkers(subObj, ident.1 = compareA, ident.2 = compareB, 
                           group.by = compareBy, test.use = testUse, 
                           recorrect_umi = recorrectUMI)
          ...
          df <- df %>% select(all_of(groupBy), sampleA, sampleB, cellNumA, cellNumB, gene,
                              p_val, p_val_adj, avg_log2FC, pct.1, pct.2, log2pctDiff)
    ...
    df <- FindAllMarkers(seuObj, only.pos = onlyPos, assay = assayName, 
                        recorrect_umi = recorrectUMI)
    df$log2pctDiff <- log2(df$pct.1 / df$pct.2)
    df <- df %>% select(cluster, gene, p_val, p_val_adj, avg_log2FC, pct.1, pct.2, log2pctDiff)
...
#' @param topGene2GO Integer, maximum genes to use for GO analysis (default: NULL)
...
#' @examples
#' \dontrun{
#' # First run DEG analysis
#' markers <- run_DEG(seurat_obj, groupBy = "seurat_clusters")
#' 
#' # Then GO analysis
#' go_results <- run_GOanalysis(markers,
#'                               groupBy = "seurat_clusters",
#'                               pCutoff = 0.05,
#'                               FCcutoff = 0.5,
#'                               prefix = "MyProject")
#' }
run_GOanalysis <- function(df, groupBy, pCutoff = 0.05, FCcutoff = 0.5, 
                           pct1Cutoff = 0.15, pct2Cutoff = 0.01, relaxFilter = FALSE, 
                           topGene2Display = 20, topGene2GO = NULL, prefix, 
                           pltHeight = 8, pltWidth = 20) {
  ...
  # Filter DEGs
  filDF <- df[df$p_val_adj < pCutoff & df$avg_log2FC > FCcutoff & 
                df$pct.1 > pct1Cutoff & df$pct.2 > pct2Cutoff, ]
  ...
  groupIDs <- sort(unique(df[[groupBy]]))
  ...
  for (i in seq(1, length(groupIDs))) {
    ...
    formatted_time <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
    cat("[", formatted_time, "] Processing", groupBy, "_", groupIDs[i], 
        "(", i, "of", length(groupIDs), ")\n")
    ...
    myColors <- c("Up_regulated" = "#8400ff", "Down_regulated" = "#dab3ff", 
                  "Un_significant" = "#b9b9b9")
    ...
    volcano_plot <- ggplot() +
      geom_hline(yintercept = c(-log10(pCutoff)), linewidth = 0.7, color = "#0059FF", 
                lty = "dashed") +
      geom_vline(xintercept = c(-FCcutoff, FCcutoff), linewidth = 0.7, color = "#0059FF", 
                lty = "dashed") +
      geom_point(data = subDF, aes(x = avg_log2FC, y = -log10(p_val_adj), color = label), 
                size = 2) +
      geom_text(aes(x = xBreak[1] * 0.95, y = -log10(pCutoff), label = pCutoff), 
               color = "red", vjust = 1.2, hjust = 0, size = 3) + 
      geom_text(aes(x = -FCcutoff, y = max(-log10(subDF$p_val_adj), na.rm = TRUE), 
                   label = -FCcutoff), angle = 90, vjust = -0.5, hjust = 1, 
               color = "red", size = 3) +
      geom_text(aes(x = FCcutoff, y = max(-log10(subDF$p_val_adj), na.rm = TRUE), 
                   label = FCcutoff), angle = 90, vjust = 1.5, hjust = 1, 
               color = "red", size = 3) +
    ...
    if (nrow(topDF) > 0) {
      volcano_plot <- volcano_plot + 
        geom_text_repel(data = topDF, aes(x = avg_log2FC, y = -log10(p_val_adj), label = gene), 
                       color = "black", max.overlaps = 50, force = 2, size = 3)
    }
    
    # GO enrichment
    if (!is.null(topGene2GO)) filSubDF <- filSubDF %>% slice_head(n = as.numeric(topGene2GO))
    
    geneSymbols <- filSubDF$gene
    ego <- enrichGO(gene = geneSymbols, OrgDb = org.Mm.eg.db, keyType = 'SYMBOL', 
